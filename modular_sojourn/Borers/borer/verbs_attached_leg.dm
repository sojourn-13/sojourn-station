...
/mob/living/simple_animal/borer/proc/speed_increase()
	...
	if(channeling && !channeling_speed_increase)
		...
		to_chat(host, "<span class='notice'>You feel the muscles in your [hostlimb == BP_R_LEG ? "right" : "left"] leg pulse.</span>")
		...
		spawn()
			...
			while(chemicals >=5 && channeling && channeling_speed_increase)
				...
				sleep(10)
			to_chat(host, "<span class='notice'>It feels like the muscles in your [hostlimb == BP_R_LEG ? "right" : "left"] leg have returned to normal.</span>")
			host.slowdown += speed_increase
			channeling = 0
			channeling_speed_increase = 0
			var/showmessage = 0
...
/mob/living/simple_animal/borer/proc/bone_talons()
	...
	if(channeling && !channeling_bone_talons)
		...
		to_chat(host, "<span class='notice'>Bony talons have grown out of your [hostlimb == BP_R_LEG ? "right" : "left"] foot.</span>")
		to_chat(src, "You begin to focus your efforts on elevating the performance of your host's [limb_to_name(hostlimb)].")
		channeling = 1
		...
		spawn()
			...
			while(chemicals >=3 && channeling && channeling_bone_talons)
				if(hostlimb == BP_R_LEG)
					if(host.has_brain_worms(BP_L_LEG))
						B = host.has_brain_worms(BP_L_LEG)
						if(B.channeling && B.channeling_bone_talons)
							synergy = 1
						else
							synergy = 0
				else
					if(host.has_brain_worms(BP_R_LEG))
						B = host.has_brain_worms(BP_R_LEG)
						if(B.channeling && B.channeling_bone_talons)
							synergy = 1
						else
							synergy = 0
				if(synergy)
					if(has_penalized_speed)
						host.slowdown += speed_penalty
						has_penalized_speed = 0
				else
					if(!has_penalized_speed)
						host.slowdown -= speed_penalty
						has_penalized_speed = 1
			...
			if(has_penalized_speed)
				if(!(B && B.channeling && B.channeling_bone_talons))
					host.slowdown += speed_penalty
			to_chat(host, "<span class='notice'>The bony talons on your [hostlimb == BP_R_LEG ? "right" : "left"] foot crumble into nothing.</span>")
			if(!(B && B.channeling && B.channeling_bone_talons))
				host.flags &= ~NO_SLIP
			channeling = 0
			channeling_bone_talons = 0

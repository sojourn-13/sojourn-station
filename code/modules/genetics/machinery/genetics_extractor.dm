...
/obj/machinery/genetics/pulper/attackby(obj/item/I, mob/user)
	if(!user.stats?.getPerk(PERK_SI_SCI) && !usr.stat_check(STAT_COG, 35) && !user.stats?.getPerk(PERK_NERD) && !usr.stat_check(STAT_BIO, 70)) //So someone that has basic chems or level up can be an assent
		to_chat(usr, SPAN_WARNING("The console announces, tired: \"You evidently can't find the on button.\""))
		return
	...
	if(stat & (NOPOWER|BROKEN))
		...
		return
	if(pulping)
		src.visible_message(SPAN_DANGER("The pulper buzzes: \"The pulper is processing.\""))
		return

	//Inserting a sample
	if(istype(I, /obj/item/reagent_containers/food/snacks/meat))
		if(meat.len >= 5)
			src.visible_message(SPAN_WARNING("The pulper announces: \"Not enough room in processing storage.\""))
			return
		if(occupant)
			src.visible_message(SPAN_WARNING("The pulper announces: \"There is already a corpse in the pulper.\""))
			return
...
/obj/machinery/genetics/pulper/affect_grab(mob/user, mob/living/target, state)
	if(stat & (NOPOWER|BROKEN))
		...
		return
	if(pulping)
		src.visible_message(SPAN_WARNING("The pulper buzzes: \"The pulper is processing.\""))
		return
	if(istype(target, /mob/living/carbon/human))
		src.visible_message(SPAN_WARNING("The pulper buzzes: \"Humans cannot be placed into the pulper.\""))
		return
	if(meat.len > 0)
		src.visible_message(SPAN_WARNING("The pulper announces: \"Not enough room in processing storage.\""))
		return
	...
	if(temp_meat_count <= 0)
		src.visible_message(SPAN_WARNING("The pulper reminds in a dead tone: \"This being has no useful genetic material.\""))
...
/obj/machinery/genetics/pulper/proc/startpulping(mob/user as mob)
	...
	spawn(gib_time) //Escape in time?
		if(occupant && (occupant.loc == src)  && occupant_meat_count && ispath(occupant_meat_type, /obj/item/reagent_containers/food/snacks/meat))
			src.visible_message(SPAN_WARNING("The pulper announces in a dead tone: \"Pulping corpse.\""))
			//big-range splatter
		...
		if(meat.len >= 0)
			src.visible_message(SPAN_WARNING("The pulper announces in a dead tone: \"Pulping meat.\""))

...
/obj/item/modular_computer/proc/try_install_component(obj/item/H, mob/living/user)
	var/found = FALSE
	var/obj/item/pc_part/CH //if it's anything other than a battery, then we need to set its holder2 var whatever the fuck that is
	if(istype(H, /obj/item/pc_part))
		CH = H
	...
	// Data disk.
	if(istype(H, /obj/item/pc_part/drive/disk))
		if(portable_drive)
	...
	else if(istype(H, /obj/item/pc_part/gps_sensor))
		if(gps_sensor)
	...
	if(insert_item(H, user))
		if(CH)
			...
			if(CH.enabled)
				CH.enabled()
			if(istype(CH, /obj/item/pc_part/drive) && enabled)
...
/obj/item/modular_computer/proc/uninstall_component(obj/item/H, mob/living/user)
	...
	var/critical = FALSE
	var/obj/item/pc_part/to_remove //If a battery, don't try to delete the snowflake vars

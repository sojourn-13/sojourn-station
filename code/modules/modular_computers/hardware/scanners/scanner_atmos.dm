/obj/item/pc_part/scanner/atmos
...
/obj/item/pc_part/scanner/atmos/can_use_scanner(mob/user, atom/target, proximity = TRUE)
	if(!..())
		return 0
	if(!target.simulated)
		return 0
...
/obj/item/pc_part/scanner/atmos/run_scan(mob/user, datum/computer_file/program/scanner/program)
	if(..())
		program.data_buffer = html2pencode(scan_data(user, user.loc)) || program.data_buffer

/obj/item/pc_part/scanner/atmos/do_on_afterattack(mob/user, atom/target, proximity)
	if(!isobj(target))
		return
	if (!scan_power_use())
		return
...
/obj/item/pc_part/scanner/atmos/proc/scan_data(mob/user, atom/target, proximity = TRUE)

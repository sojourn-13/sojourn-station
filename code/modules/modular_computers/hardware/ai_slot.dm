...
/obj/item/pc_part/ai_slot/proc/update_power_usage()
	if(!stored_card || !stored_card.carded_ai)
		power_usage = power_usage_idle
		return
	power_usage = power_usage_occupied

/obj/item/pc_part/ai_slot/attackby(obj/item/W, mob/user)
	if(..())
		return 1
...
/obj/item/pc_part/ai_slot/Destroy()
	if(stored_card)
		stored_card.forceMove(drop_location())
	return ..()

...
/mob/living/proc/damage_through_armor(
	...
	//Here we split damage in two parts, where armor value will determine how much damage will get through
		...
		if(!(damagetype == HALLOSS ))
			var/agony_gamage = round( ( effective_damage * armor_effectiveness * ARMOR_HALLOS_COEFFICIENT * max(0.5, (get_specific_organ_efficiency(OP_NERVE / 1.5, def_zone) / 100)) / 100))

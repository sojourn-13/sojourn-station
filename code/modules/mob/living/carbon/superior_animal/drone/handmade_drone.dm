...
/mob/living/carbon/superior_animal/handmade/examine(mob/user)
	...
	if(iscarbon(user) || issilicon(user))
		...
		if(robotics_expert) // Are we an expert in robots?
			...
			to_chat(user, SPAN_WARNING("It's wounds are mounting."))
			...
			to_chat(user, SPAN_WARNING("It has acrewed some lasting combat."))
		else if (health < maxHealth * 0.80)
			to_chat(user, SPAN_WARNING("It has minor damage done to it."))

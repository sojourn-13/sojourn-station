...
/mob/living/carbon/superior_animal/giant_spider/hunter/shocker/UnarmedAttack(var/atom/A, var/proximity)
	...
	if(isliving(A))
		...
		if(istype(targetted_mob) && prob(shock_chance))
			...
			if(iscarbon(targetted_mob)) // only carbons can be affected by electricity
				var/mob/living/carbon/targetted_carbon = targetted_mob
				targetted_carbon.electrocute_act(electrocution_amount, src)
			else

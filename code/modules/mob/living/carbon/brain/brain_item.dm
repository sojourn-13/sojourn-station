...
/obj/item/organ/internal/brain/slime
	...
	var/mob/living/carbon/human/host_copy
...
/obj/item/organ/internal/brain/slime/attackby(obj/item/I, mob/user)
	if(host_copy && istype(I, /obj/item/reagent_containers/syringe) && !regenerating)
...
/obj/item/organ/internal/brain/slime/removed_mob()
	host_copy = new(src, FORM_SLIME, FORM_SLIME) // Copy the body as backup. I don't think the appearance is saved, but it's fine for now. -R4d6
	host_copy?.real_name = owner.real_name
	..()
...
/obj/item/organ/internal/brain/slime/proc/regen_body()
	...
	if(host_copy)
		forceMove(host_copy, get_turf(src)) // Eject the copy
		replaced(host_copy.get_organ(BP_CHEST)) // Put the core in
		for(var/O in host_copy.organ_list_by_process(BP_BRAIN)) // Remove any surplus cores
			if(O != src) // It's not our brain.
				qdel()

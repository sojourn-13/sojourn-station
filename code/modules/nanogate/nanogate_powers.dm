...
/obj/item/organ/internal/nanogate/proc/stand_damage()
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/stand_damage
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
...
/obj/item/organ/internal/nanogate/proc/stand_health()
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/stand_health
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/stand_armor()
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/stand_armor
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/stand_repair()
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/stand_repair
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/autodoc_mode()
	set category = "Nanogate Powers"
	set name = "Activate Protocol - Autodoc (1)"
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/autodoc_mode
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/radio_mode()
	set category = "Nanogate Powers"
	set name = "Activate Protocol - Radio (1)"
	set desc = "Spend some of your nanites to activate a protocol in your bot."
	nano_point_cost = 1
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/radio_mode
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/food_mode()
	set category = "Nanogate Powers"
	set name = "Activate Protocol - Food (1)"
	set desc = "Spend some of your nanites to activate a protocol in your bot."
	nano_point_cost = 1
	...
	if(Stand) // Do they have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/food_mode
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else
		to_chat(owner, "You do not have a nanobot to upgrade!")
...
/obj/item/organ/internal/nanogate/proc/nanite_regen()
	...
	if(!owner.stats.getPerk(PERK_NANITE_REGEN)) // Do they already have the bot?
		if(pay_power_cost(nano_point_cost))
			...
			owner.verbs -= /obj/item/organ/internal/nanogate/proc/nanite_regen
		else
			to_chat(owner, "You do not have any nanites to assign to that task.")
	else

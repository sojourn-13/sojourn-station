		...
		if(3) //Every third tick it plays a sound and RNG's a flavor text
			...
			if(buckled_mob)
				...
				if(prob(50))
					...
					else if(sabotaged == 1 && prob(99))
						buckled_mob.Weaken(3)
						unbuckle_mob()
						var/obj/item/stack/cable_coil/C = new(get_turf(src))
						C.amount = 25
						qdel(src)
...
/obj/structure/noose/bullet_act(obj/item/projectile/P)
	if(prob(30))
		visible_message("\The [src] gets split by \the [P]!")
		unbuckle_mob()
		qdel(src)

/obj/structure/noose/verb/sabotage()
	set src in range(1)
	set name = "Sabotage"
	set category = "Object"
	if(sabotaged == 1)
	...
	if(isWirecutter(usr.get_active_hand()))
		sabotaged = 1
		to_chat(usr, "<span class='notice'>You succesfully sabotaged the noose!</span>")
	else
		to_chat(usr, "You need a wirecutter to do that!")

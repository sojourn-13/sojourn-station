	...
	if(user.unEquip(src,T))
		...
		switch(ndir)
			...
			if(WEST)
				...
				pixel_y = 0
				to_turn = 90
		add_new_transformation(/datum/transform_type/modular, list(rotationarg = to_turn, flagarg = WALLMOUNT_PLACED_ROTATION_TRANSFORMATION, priority = WALLMOUNT_PLACED_ROTATION_TRANSFORMATION_PRIORITY))

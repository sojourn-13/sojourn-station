...
/client/proc/handle_commandbar_typing(href_list)
	if(get_preference_value(/datum/client_preference/show_typing_indicator) != GLOB.PREF_SHOW) //check pref
		return
...
/client/proc/start_typing()
	...
	if(get_preference_value(/datum/client_preference/show_typing_indicator) != GLOB.PREF_SHOW || !HAS_TRAIT(client_mob, TRAIT_THINKING_IN_CHARACTER))
		return FALSE

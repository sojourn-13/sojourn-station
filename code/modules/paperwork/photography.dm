...
/obj/item/photo/proc/show(mob/user as mob)
	...
	var/photo_showing = "window=book;size=[64*photo_size]x[scribble ? 400 : 64*photo_size]"
	var/html = HTML_SKELETON_RAW("<title>[name]</title>","<body style='overflow:hidden;margin:0;text-align:center'>" \
		+ "<img src='tmp_photo_[id].png' width='[64*photo_size]' style='-ms-interpolation-mode:nearest-neighbor' />" \
		+ "[scribble ? "<br>Written on the back:<br><i>[scribble]</i>" : ""]"\
		+ "</body>")
	user << browse(html, photo_showing)
	onclose(user, "book")
	return

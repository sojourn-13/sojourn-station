...
/datum/cooking_with_jane/recipe_step/use_grill/calculate_quality(var/obj/used_item, var/datum/cooking_with_jane/recipe_tracker/tracker)
	...
	var/overcooking = round((time - container.grill_data[heat]), 10 SECONDS)/ 1 SECONDS

	var/good_cooking = round(time/3 SECONDS) - bad_cooking - overcooking + our_grill.quality_mod

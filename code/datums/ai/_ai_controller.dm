...
/datum/ai_controller/proc/on_sentience_gained()
	...
	if(!continue_processing_when_client)
		INVOKE_ASYNC(src, .proc/set_ai_status, AI_STATUS_OFF) //Can't do anything while player is connected
	RegisterSignal(pawn, COMSIG_MOB_LOGOUT, .proc/on_sentience_lost)

/datum/ai_controller/proc/on_sentience_lost()
	SIGNAL_HANDLER
	UnregisterSignal(pawn, COMSIG_MOB_LOGOUT)
	INVOKE_ASYNC(src, .proc/set_ai_status, AI_STATUS_ON) //Can't do anything while player is connected
	RegisterSignal(pawn, COMSIG_MOB_LOGIN, .proc/on_sentience_gained)

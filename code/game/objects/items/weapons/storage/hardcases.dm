...
/obj/item/gunbox/traumatizedteam/attack_self(mob/living/user)
	...
	if(!stamped)
		stamped = TRUE
		var/list/options = list()
		// Making the "Bullpip" an attractive alternative by including 1 more mag and a silencer
		...
		options["Soteria \"Sprocket\" lasgun"] = list(/obj/item/gun/energy/cog/sprocket/preloaded,/obj/item/cell/medium/moebius/high) // Moved the Format Bound extra cell here.

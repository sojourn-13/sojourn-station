...
/datum/event/grid_check/announce()
	command_announcement.Announce("Abnormal activity detected in the colony's powernet. As a precautionary measure, the power will be shut off for an indeterminate duration.", "Automated Grid Check", new_sound = 'sound/AI/poweroff.ogg')



/proc/power_failure(var/announce = 1, var/severity = 2, var/list/affected_z_levels)
	if(announce)
		command_announcement.Announce("Abnormal activity detected in the colony's powernet. As a precautionary measure, the power will be shut off for an indeterminate duration.", "Critical Power Failure", new_sound = 'sound/AI/poweroff.ogg')
...
/proc/power_restore(var/announce = 1)
	...
	if(announce)
		command_announcement.Announce("Power has been restored to colony. We apologize for the inconvenience.", "Power Systems Nominal", new_sound = 'sound/AI/poweron.ogg')
	for(var/obj/machinery/power/apc/C in SSmachines.machinery)
		C.failure_timer = 0
		if(C.cell)

...
/datum/storyteller/proc/can_start(var/announce = FALSE)	//when TRUE, proc should output reason, by which it can't start, to world
	...
	if(announce)
		if(!engineer && !command)
			to_chat(world, "<b><font color='[tcol]'>A Command Officer and Guild Member are required to start the round.</font></b>")
		else if(!engineer)
			to_chat(world, "<b><font color='[tcol]'>A Guild Member is required to start the round.</font></b>")
		else if(!command)
			to_chat(world, "<b><font color='[tcol]'>A Command Officer is required to start the round.</font></b>")

#define PRONE_MATRIX_ROTATE 90

#define PRONE_TRANSFORM "prone_transform"
#define RANDOM_ROTATION_TRANSFORM "random_rotaton_transform"

#define HUMAN_PRONE_TRANSFORM "human_prone_transformation"
#define HUMAN_SIZE_SCALING_TRANSFORM "human_size_scaling_transform"

#define AMERIDIAN_TRANSFORM "ameridian_transform"
#define CRYSTAL_RESIZING_TRANSFORM "crystal_resizing_transform"
#define AMERIDIAN_SPIRE_DEFAULT_RESIZE_TRANSFORM "ameridian_spire_default_resize_transform"

#define MODULAR_BASE_TRANSFORM_DO_NOT_USE "DO_NOT_USE_transform"

#define SHARD_VARIABLE_SIZE_TRANSFORM "shard_variable_size_transform"

#define FLASHLIGHT_LIGHT_SPOT_ROTATION_TRANSFORM "flashlight_rotation_transform"

#define VTM_CLIMB_ROTATION_TRANSFORM "VTM_climb_rotation_transform"
#define VTM_CLIMB_MAG_ROTATION_TRANSFORM "VTM_climb_mag_rotation_transform"
#define VTM_JUMP_TRANSFORM "VTM_jump_transform"

#define PROJECTILE_GENERIC_TRANSFORM "projectile_generic_transform"

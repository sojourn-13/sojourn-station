...
/datum/evacuation_controller/proc/call_evacuation(var/mob/user, var/_emergency_evac, var/forced, var/skip_announce, var/autotransfer)
	...
	if(emergency_evacuation)
		...
		if(!skip_announce)
			priority_announcement.Announce(replacetext(replacetext(maps_data.shuttle_called_message, "%dock_name%", "[dock_name]"),  "%ETA%", "[round(get_eta()/60)] minute\s"))
...
/datum/evacuation_controller/proc/cancel_evacuation()
	...
	if(emergency_evacuation)
		...
		priority_announcement.Announce(maps_data.shuttle_recall_message)
...
/datum/evacuation_controller/proc/finish_preparing_evac()
	...
	if (emergency_evacuation)
		evac_waiting.Announce(replacetext(maps_data.emergency_shuttle_docked_message, "%ETD%", "[estimated_time] minute\s"), new_sound = sound('sound/effects/Evacuation.ogg', volume = 35))
	else

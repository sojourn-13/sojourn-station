...
/proc/qdel(datum/to_delete, force=FALSE, ...)
	...
	if(isnull(to_delete.gc_destroyed))
		if (LEGACY_SEND_SIGNAL(to_delete, COMSIG_PARENT_PREQDELETED, force)) // Give the components a chance to prevent their parent from being deleted
			return
		to_delete.gc_destroyed = GC_CURRENTLY_BEING_QDELETED
		var/start_time = world.time
		var/start_tick = world.tick_usage
		LEGACY_SEND_SIGNAL(to_delete, COMSIG_PARENT_QDELETING, force) // Let the (remaining) components know about the result of Destroy
		SEND_SIGNAL(to_delete, COMSIG_QDELETING_NEW, force)

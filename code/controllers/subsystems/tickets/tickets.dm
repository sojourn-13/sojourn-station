...
/datum/controller/subsystem/tickets/fire()
	...
	if(LAZYLEN(stales))
		...
		message_adminTicket("<span class='[span_class]'>Tickets [report] have been open for over [TICKET_TIMEOUT / 600] minutes. Changing status to stale.</span>")
...
/datum/controller/subsystem/tickets/proc/openTicket(N)
	...
	if(T.ticketState != TICKET_OPEN)
		...
		message_adminTicket("<span class='[span_class]'>[usr.client] / ([usr]) re-opened [ticket_name] number [N]</span>")
...
/datum/controller/subsystem/tickets/proc/resolveTicket(N)
	...
	if(T.ticketState != TICKET_RESOLVED)
		...
		message_adminTicket("<span class='[span_class]'>[usr.client] / ([usr]) resolved [ticket_name] number [N]</span>")
...
/datum/controller/subsystem/tickets/proc/convert_ticket(datum/ticket/T)
	...
	message_adminTicket("<span class='[span_class]'>[C] has converted ticket number [T.ticketNum] to a [other_ticket_name] ticket.</span>")
...
/datum/controller/subsystem/tickets/proc/closeTicket(N)
	...
	if(T.ticketState != TICKET_CLOSED)
		...
		message_adminTicket("<span class='[span_class]'>[usr.client] / ([usr]) closed [ticket_name] number [N]</span>")
...
/datum/controller/subsystem/tickets/proc/takeTicket(var/index)
	if(assignStaffToTicket(usr.client, index))
		...
		message_adminTicket("<span class='[span_class]'>[usr.client] / ([usr]) has taken [ticket_name] number [index]</span>")
...
/datum/controller/subsystem/tickets/proc/unassignTicket(index)
	...
	if(T.staffAssigned != null && (T.staffAssigned == usr.client || alert("Ticket is already assigned to [T.staffAssigned]. Do you want to unassign it?","Unassign ticket","No","Yes") == "Yes"))
		...
		message_adminTicket("<span class='[span_class]'>[usr.client] / ([usr]) has unassigned [ticket_name] number [index]</span>")

...
const INDEXED_DB_NAME = 'soj-tgui';
...
class HubStorageBackend {
  ...
  get(key) {
    const value = window.hubStorage.getItem('sojourn-' + key);
  ...
  set(key, value) {
    window.hubStorage.setItem('sojourn-' + key, JSON.stringify(value));
  }

  remove(key) {
    window.hubStorage.removeItem('sojourn-' + key);
  }

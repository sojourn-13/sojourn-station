    ...
    <Window width={400} height={640}>
      <Window.Content scrollable>
        <Section>
          ...
          <Box>
            Choose one of the following open/valid positions.
          </Box>
            ...
            return (
              ...
              >
                {filtered_jobs.map(job => (
                  <Button fluid key={job.title} onClick={() => act("join", { job: job.title })}>
                    <Stack>
                      <Stack.Item grow>{job.title}</Stack.Item>
                      <Stack.Item>({job.current_positions}) (Active: {job.active})</Stack.Item>
                    </Stack>
                  </Button>
                ))}
              </Section>
